...
function setColorBounds(color) {
  ...
  upper['s'] *= 255;
  upper['v'] *= 255;
  // Wrap around if upper is 0.
  // Could cause problems theoretically, but is OK for demonstration purposes.
  if (Math.floor(upper['h']) === 0) {
    upper['h'] = 179;
  }
...
handler.initialize = function (bayeux, sessionStore) {
  ...
  // Make faye server push-only. Authenticate subscription requests.
  bayeux.addExtension({
...
handler.start = function () {
  ...
  webcamProcess.stderr.on('data', function (data) {
    ...
    if (data === 'Exceeded') {
      ...
      if (props.experiment && props.experiment.contact !== 'none'
          && !props.colorAlerted) {
